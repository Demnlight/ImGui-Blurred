...
bool ImGui::CloseButton( ImGuiID id, const ImVec2& pos, ImVec2 size ) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;

	// Tweak 1: Shrink hit-testing area if button covers an abnormally large proportion of the visible region. That's in order to facilitate moving the window away. (#3825)
	// This may better be applied as a general hit-rect reduction mechanism for all widgets to ensure the area to move window is always accessible?
	const ImRect bb( pos, pos + ImVec2( size.x, size.y ) );
	...
	ImU32 col = ( held || hovered ) ? ImColor( 224, 36, 55 ) : ImGuiCol_ButtonHovered;
...
bool ImGui::MaximizeButton( ImGuiID id, const ImVec2& pos, ImVec2 size ) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	...
	// Render
	// FIXME: Clarify this mess
	ImU32 col = GetColorU32(( held || hovered ) ? ImGuiCol_FrameBgHovered : ImGuiCol_Button, 0.5f );
	ImVec2 center = bb.GetCenter( );
	if ( hovered )
		window->DrawList->AddRectFilled( bb.Min, bb.Max, col );

	float cross_extent = g.FontSize * 0.7071f - 1.0f;
	ImU32 cross_col = GetColorU32( ImGuiCol_Text );
	window->DrawList->AddRect( bb.Min + ImVec2( cross_extent, cross_extent ), bb.Max - ImVec2( cross_extent, cross_extent ), cross_col, 1.0f );
	
	return pressed;
}
bool ImGui::MinimizeButton( ImGuiID id, const ImVec2& pos, ImVec2 size ) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
